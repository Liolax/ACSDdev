  ...
  return (
    <div>
      <ul className="buyer-dashboard__cart-list">
        {visibleItems.map(item => (
          <li key={item.id} className="buyer-dashboard__cart-item">
            ...
            <span>
              {item.name} - 
              <span className="buyer-dashboard__cart-price"> ${item.price.toFixed(2)} x {item.quantity}</span>
            </span>
      ...
      <div className="buyer-dashboard__cart-summary">
        <p>Total: <span className="buyer-dashboard__cart-total"> ${total.toFixed(2)}</span></p>
