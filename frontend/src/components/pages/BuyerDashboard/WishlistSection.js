        ...
        {visibleItems.map(item => (
          <li key={item.id} className="buyer-dashboard__wishlist-item">
            ...
            <span className="buyer-dashboard__wishlist-item-info">
              {item.name} – <span className="buyer-dashboard__wishlist-price">${item.price.toFixed(2)}</span>
            </span>
            <div className="buyer-dashboard__wishlist-actions">
              <button onClick={() => onMoveToCart(item)} className="button buyer-dashboard__button--sm">To Cart</button>
              <button onClick={() => onRemove(item.id)} className="button buyer-dashboard__button--sm">Delete</button>
            </div>
          </li>
        ))}
      </ul>
